...
const Models = require("../fixtures/db_connection/models/index");
...
class Database {
	 ...
	 *
	 * @param {String} table - name of the table (defined in ../fixtures/db_connection/models/index.js)
	 ...
	 *  - sort:		{type: "String", optional: true}   --> ex:  '-created_at'
	 *
	...
	find(ctx, search) {
		...
		var populate = (search) ?  search.populate || { } : { };
		var sort = (search) ?  search.sort || '-created_at' : '-created_at';
		var offset = (search) ? ((typeof search.offset === "number") ? search.offset : undefined) : undefined;
		...
		return this.parameterValidator({ctx: ctx})
			...
			.then( () => {
				...
				if (search.populate) {
					findObj.populate = search.populate;
				}

				return ctx.call(`${this.table}.find`, findObj)
			})
		...
		return this.parameterValidator({ctx: ctx, entity: entity})
			.then( () => ctx.call(`${this.table}.insert`, {
					entity: Object.assign(entity, {
						created_at: new Date(),
						created_by: ctx.meta.user ? ctx.meta.user.id : null
					})
				})
			)
			.then( (res) => Promise.resolve({
	...
	insertMany(ctx, entities) {
		...
		return this.parameterValidator({ctx: ctx, entities: entities})
			...
			.then( (final_entities) => ctx.call(`${this.table}.insertMany`, {
				entities: Object.assign(final_entities, {
					created_at: new Date(),
	...
	updateById(ctx, id, update) {
		var userid = '';
		if (Object.keys(ctx.meta).length === 0 && ctx.meta.constructor === Object) {
			userid = 1;
		} else {
	...
	/**
	 * 	Update the entity with the given query
	 *
	 * @param {Object} ctx - Will serve to call a service action: ctx.call
	 * @param {String} query Object with fields to where --> ex:  { username: "user_2" }
	 * @param {Object} update - Object with fields to update --> ex:  { username: "user_2" }
	 *
	 * @returns {Promise}
	 */
	updateBy(ctx, id, update, query) {
		return this.parameterValidator({ctx: ctx, id: id, update: update})
			.then( () => this.findOne(ctx, query))
			.then( (res) => ctx.call(`${this.table}.updateById`, {
				id: res.data.id,
				update: Object.assign(update, {
					updated_at: new Date(),
					updated_by: ctx.meta.user ? ctx.meta.user.id : null
				})
			}))
			.then( () => this.find(ctx, query))
			.then( (res) => Promise.resolve({
				code: 200,
				name: "Operation Successful",
				message: `Update Done`,
	...
	updateMany(ctx, query, update) {
		...
		return this.parameterValidator({ctx: ctx, query: query, update: update})
			...
			.then( (res) => Promise.all( res.data.map( (item) => this.updateById(ctx, item.id, Object.assign(update, {
				updated_at: new Date(),
				updated_by: userid
			})) ) ) )
