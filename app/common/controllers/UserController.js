 ...
 *
 * @annotation Login
 * @permission get,changePassword,user_changePassword,agent_changePassword,admin_changePassword,remove
 * @whitelist get
 */
module.exports = {
  create: function (ctx) {
  ...
  admin_changePassword: function (ctx) {
    ...
    return (
      this.verifyIfLogged(ctx)
        .then(() =>
          ctx.call("auth.verifyadmin_change_Password", {
            id: ctx.params.id,
            password: ctx.params.oldpassword,
            newpassword: ctx.params.newpassword,
          })
        ...
        .then(async(res) => {
          if (ctx.params.newpassword.localeCompare(ctx.params.confirmpassword) == 0) {
            this.generateHash(ctx.params.newpassword).then(async(res) => {
              ...
              dlMailer.sendMail(ctx, ConstantsMailTemplate.UserAgencyChangePassword, UserDetail.data.email, replacements, Constants.AdminMailId);
